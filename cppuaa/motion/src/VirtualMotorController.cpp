...
bool VirtualMotorController::connect(const std::string& ip, int p) {
  if (is_connected) {
    std::cout << "[VIRTUAL] Motor controller is already connected" << std::endl;
    return true;
  }
  ...
  std::cout << "[VIRTUAL] Connecting to motor controller at " << ip_address << ":" << port << "..." << std::endl;
  ...
  position_update_thread = std::thread(&VirtualMotorController::update_position_thread, this);

  std::cout << "[VIRTUAL] Motor controller connected successfully" << std::endl;
  return true;
}

bool VirtualMotorController::disconnect() {
  if (!is_connected) {
    std::cout << "[VIRTUAL] Motor controller is already disconnected" << std::endl;
    return true;
  }

  std::cout << "[VIRTUAL] Disconnecting from motor controller..." << std::endl;
  ...
  std::cout << "[VIRTUAL] Motor controller disconnected successfully" << std::endl;
  return true;
}

void VirtualMotorController::update_position_thread() {
  ...
  while (is_connected) {
    ...
    if (is_moving) {
      ...
      if (reached_target) {
        is_moving = false;
        std::cout << "[VIRTUAL] Target position reached" << std::endl;
      }
    }
  }
}

bool VirtualMotorController::get_position(Position6axis& out_position) {
  if (!is_connected) {
    std::cout << "[VIRTUAL] Error: Motor controller is not connected" << std::endl;
    return false;
  }
...
bool VirtualMotorController::move_to_point(const Position6axis& target) {
  if (!is_connected) {
    std::cout << "[VIRTUAL] Error: Motor controller is not connected" << std::endl;
    return false;
  }
  ...
  //std::cout << "[VIRTUAL] Moving to position: ["
  //  << target.x << ", "
  //  << target.y << ", "
  //  << target.z << ", "
  //  << target.u << ", "
...
bool VirtualMotorController::is_controller_connected() const {
  return is_connected;
}

bool VirtualMotorController::is_controller_moving() const {
  return is_moving;
}

void VirtualMotorController::set_movement_parameters(double speed, double acceleration, double tolerance) {
  std::lock_guard<std::mutex> lock(position_mutex);
...
void VirtualMotorController::set_simulated_position(const Position6axis& position) {
  std::lock_guard<std::mutex> lock(position_mutex);
...
Position6axis VirtualMotorController::interpolate_position(const Position6axis& start, const Position6axis& end, double t) {
...
void VirtualMotorController::add_position_noise(Position6axis& pos, double magnitude) {
...
std::string VirtualMotorController::get_status_json() const {
